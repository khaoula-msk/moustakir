...
public class inscription extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Personne p = new Personne(request.getParameter("nom"), request.getParameter("prenom"), request.getParameter("email"), request.getParameter("mdp"), request.getParameter("cne"), null);
		 try {
		...
		if(res) {
			...
			{response.sendRedirect("http://localhost:8080/moustakir.khaoula/authentification");
				...
				response.sendRedirect("http://localhost:8080/moustakir.khaoula/inscription");
			}
		}else {
