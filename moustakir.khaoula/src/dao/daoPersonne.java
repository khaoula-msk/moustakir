...
/* public static int inscription(Personne p) throws ClassNotFoundException, SQLException {
	...
	public static int AjouterEtudiant(Personne p) throws ClassNotFoundException, SQLException {
		Connexion.Connect();
		int res = Connexion.Maj("INSERT INTO `personne`(`nom`, `prenom`, `email`, `mdp`,`cne`, `role`) VALUES ('"+p.getNom()+"','"+p.getPrenom()+"','"+p.getEmail()+"','"+p.getMdp()+"','"+p.getCne()+"','etudiant')");
		Connexion.Disconnect();
		return res;
	}
	public static ArrayList<Personne> listeAuteurs() throws SQLException, ClassNotFoundException{
		...
		while(res.next()) {
			Personne p =new Personne(res.getInt(1), res.getString(2), res.getString(3), res.getString(4), res.getString(5), res.getString(6), Role.AUTEUR);
			personnes.add(p);
		}
		Connexion.Disconnect();
		return personnes;
	...
	public static ArrayList<Personne> listeEtudiants() throws SQLException, ClassNotFoundException{
		ArrayList<Personne> personnes = new ArrayList<Personne>();
		Connexion.Connect();
		ResultSet res = Connexion.Select("select * from personne");
		while(res.next()) {
			Personne p =new Personne(res.getInt(1), res.getString(2), res.getString(3), res.getString(4), res.getString(5), res.getString(6), Role.ETUDIANT);
			personnes.add(p);
		}
		Connexion.Disconnect();
		return personnes;
	...
	public static Personne PersonneParId(int id) throws ClassNotFoundException, SQLException {
		...
		if(res.next()) {
			p=new Personne(res.getInt(1), res.getString(2), res.getString(3), res.getString(4), res.getString(5), null, null);
	...
	public static Personne authentification(String login,String password) throws SQLException, ClassNotFoundException {
		...
		if(res.next()) {
			p=new Personne(res.getInt(1), res.getString(2), res.getString(3), res.getString(4), res.getString(5),  res.getString(6), null);
			if(res.getString(6).equals("etudiant")) {
				p.setRole(Role.ETUDIANT);
			} else if(res.getString(6).equals("admin")) {
				p.setRole(Role.ADMIN);
